                ...
                child: Column(
                  children: [
                    ReorderableListView(
                      shrinkWrap: true,
                      children: <Widget>[
                        for (int index = 0; index < model.accManager.allAccounts.length; index++)
                          Container(
                            key: Key('$index'),
                            ...
                            child: Row(
                              children: [
                                ...
                                SizedBox(width: 10),
                                Text('${model.accManager.allAccounts[index].accountAlias}'),
                                Spacer(),
                                PopupMenuButton<int>(
                                    padding: EdgeInsets.zero,
                                    icon: Icon(Icons.more_vert_outlined),
                                    onSelected: (val) {
                                      model.onSelectedPopupMenu(val, index, context);
                                    },
                                    itemBuilder: (context) => [
                                          PopupMenuItem<int>(
                                            value: 1,
                                            child: Text(S.of(context).rename, style: Theme.of(context).textTheme.subtitle1),
                                          ),
                                          if (model.accManager.allAccounts.length > 1)
                                            PopupMenuItem<int>(
                                              value: 2,
                                              child: Text(S.of(context).logOut, style: Theme.of(context).textTheme.subtitle1),
                                            ),
                                        ])
                              ],
                            ),
                      ...
                      onReorder: (int oldIndex, int newIndex) {
                        ...
                        final acc = model.accManager.allAccounts.removeAt(oldIndex);
                        model.accManager
                    ...
                    Button(
                        value: 'Add wallet +',
                        onTap: () {
                          Navigator.of(context).push(MaterialPageRoute(builder: (_) => StartScreen(addWallet: true)));
                        })
                  ],
                )));
      },
...
class AccountsSettingsModel extends BaseViewModel {
    ...
    var newAlias = await showDialog<String>(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Column(
          children: [
            TextFormField(
              decoration: generalTextFieldDecor(context, hintText: S.of(context).newAccountName),
        ...
        actions: <Widget>[
          SimpleDialogOption(
            child: Text(
              S.of(context).cancel,
              style: Theme.of(context).textTheme.subtitle2!.copyWith(color: AppColors.red),
            ),
            onPressed: () {
          ...
          SimpleDialogOption(
            child: Text(
              ...
              style: Theme.of(context).textTheme.subtitle2!.copyWith(color: AppColors.active),
            ),
            onPressed: () {
  ...
  deleteAccount(BuildContext context, int index) async {
    var confirmed = (await showConfirmationDialog(S.of(context).logOutQuestion, S.of(context).checkSavedMnemonicSingle)).confirmed;
    if (confirmed) {
