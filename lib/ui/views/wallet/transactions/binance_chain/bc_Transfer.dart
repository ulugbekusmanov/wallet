...
class BCTransferScreen extends StatelessWidget {
  ...
  int accountFrom;
  BCTransferScreen(this.balance, this.accountFrom, {Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return BaseView<BCTransferModel>(
      onModelReady: (model) async {
        ...
        model.bnbBalance = model.account.bc_bep2_Balances.firstWhere((b) => b.token.symbol == 'BNB');
        model.init();
      },
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    GestureDetector(
                                      ...
                                      onTap: () async {
                                        var addressBookPick = await Navigator.of(context).push<String?>(PageTransition(child: AddressBookPickerScreen(TokenNetwork.BinanceChain), type: PageTransitionType.rightToLeft));
                                    ...
                                    Stack(children: [
                                      TextFormField(
                                        ...
                                        decoration: generalTextFieldDecor(context, hintText: 'Memo', paddingRight: 86),
                                      ),
                                      Positioned(
                                        right: 46,
                                        top: 10,
                                        child: textFieldActionsButton(child: gradientIcon(AppIcons.book_open(22)), onTap: () => model.pasteMemo(context)),
                                      ),
                                      Positioned(
                                        right: 10,
                                        top: 10,
                                        child: textFieldActionsButton(child: gradientIcon(AppIcons.qr_code_scan(22)), onTap: () => model.scanMemoQr(context)),
                                      ),
                                    ]),
                                    Padding(
                                      padding: const EdgeInsets.only(left: 16, top: 4),
                                      child: Text(S.of(context).memoInfoDialogText, style: Theme.of(context).textTheme.caption),
                                    ),
                                  ],
                                ),
                              ),
