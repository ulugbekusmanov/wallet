...
class BSCMulticallInteractor {
  ...
  BSCMulticallInteractor([this.provider]) {
    contract = DeployedContract(multicallContractAbi, EthereumAddress.fromHex('0x1Ee38d535d541c55C9dae27B12edf090C608E6Fb'));
  ...
  Future<List<EtherAmount>> getBalances(List<EthereumAddress> addresses, List<WalletToken> tokens) async {
    final balanceOfFunc = bep20BasicContractAbi.functions.firstWhere((f) => f.name == 'balanceOf');
  ...
  Future<List<dynamic>> aggregate(List<dynamic> args) async {
    return (provider ?? ENVS.BSC_ENV!).call(contract: contract, function: aggFunc, params: args);
  }
}
