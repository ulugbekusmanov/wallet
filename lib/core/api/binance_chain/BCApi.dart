...
import 'package:tbccwallet/core/authentication/wallets/BinanceChainWallet.dart';
...
class BinanceChainApi {
  ...
  Future<ApiResponse<List<Candlestick>>> getCandlestickBarsMini(String symbol, {CandlestickInterval interval = CandlestickInterval.INTERVAL_1h}) async {
    return ApiResponse.fromBC(await _bclient.getCandlestickBarsMini(symbol: symbol, interval: interval));
  }

  Future<ApiResponse<List<Candlestick>>> getCandlestickBars(String symbol, {CandlestickInterval interval = CandlestickInterval.INTERVAL_1h}) async {
    return ApiResponse.fromBC(await _bclient.getCandlestickBars(symbol: symbol, interval: interval));
  }
  ...
  Future<ApiResponse<TxPage>> getTransactions(String address, String symbol) async {
    var resp = await _bclient.getTransactions(address: address, startTime: DateTime.now().subtract(Duration(days: 85)).millisecondsSinceEpoch, txAsset: symbol);
    return ApiResponse.fromBC(resp);
  }
  ...
  Future<ApiResponse<List<Transaction>>> sendToken(BCWallet wallet, String toAddress, String symbol, Decimal amount, String memo, {bool sync = false}) async {
  ...
  Future<ApiResponse<List<Transaction>>> cancelOrder(CancelOrderMsg order) async {
    await order.wallet!.reload_account_sequence();
  ...
  Future<ApiResponse<List<TickerStatistics>>> getTickers({String? symbol}) async {
    var resp = await _bclient.getTickerStats24hr(symbol: symbol);

    return ApiResponse.fromBC(resp);
  }

  Future<ApiResponse<List<TickerStatistics>>> getMiniTickers({String? symbol}) async {
