...
class TickersService {
  ...
  Future<void> loadSimpleTickers(List<String> ids, String fiatCurr) async {
    simpleTickers = (await _coingeckoApi.loadTickers(ids: ids..removeWhere((element) => element == '-1'), include24hchange: true, include24hvol: true, vsCurrencies: [fiatCurr])).load;
  }

  Future<void> loadBCDexTickers() async {
    var resp = await Future.wait([_bcApi.getTickers(), _bcApi.getMiniTickers()]);
